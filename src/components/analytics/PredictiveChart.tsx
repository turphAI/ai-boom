        ...
        <div className={`${isZoomed ? 'scale-110 transform' : ''} transition-transform duration-200`}>
          ...
          >
            ...
            <svg className="absolute inset-0 w-full h-full" style={{ pointerEvents: 'none' }}>
              ...
              <polyline
                points={data.map((point, i) => {
                  ...
                  const y = 100 - scaleValue(point.value)
                  return `${x},${y}`
                }).join(' ')}
                fill="none"
                stroke={color}
              ...
              {prediction && showPrediction && (
                <polyline
                  points={prediction.predictions.map((pred, i) => {
                    const x = ((data.length + i) / (data.length + predictionPeriods - 1)) * 100
                    const y = 100 - scaleValue(pred.value)
                    return `${x},${y}`
                  }).join(' ')}
                  fill="none"
                  stroke="#ef4444"
              ...
              {prediction && showPrediction && (
                <path
                  d={`
                    M ${(data.length / (data.length + predictionPeriods - 1)) * 100},${100 - scaleValue(prediction.predictions[0].upper)}
                    ${prediction.predictions.map((pred, i) => {
                      const x = ((data.length + i) / (data.length + predictionPeriods - 1)) * 100
                      const y = 100 - scaleValue(pred.upper)
                      return `L ${x},${y}`
                    }).join(' ')}
                    ${prediction.predictions.reverse().map((pred, i) => {
                      const x = ((data.length + predictionPeriods - 1 - i) / (data.length + predictionPeriods - 1)) * 100
                      const y = 100 - scaleValue(pred.lower)
                      return `L ${x},${y}`
                    }).join(' ')}
                    Z
                  `}
            ...
            {data.map((point, index) => (
              <div
                key={index}
                className="absolute w-3 h-3 bg-blue-500 rounded-full cursor-pointer hover:scale-150 transition-transform border-2 border-white"
                style={{
                  left: `${(index / (data.length - 1)) * 100}%`,
                  top: `${100 - scaleValue(point.value)}%`,
                  transform: 'translate(-50%, -50%)',
                  backgroundColor: color
                }}
                onClick={() => setSelectedPoint(point)}
                title={`${formatDate(point.timestamp)}: ${point.value.toFixed(2)} ${unit}`}
              />
            ))}
            ...
            {prediction && showPrediction && prediction.predictions.map((pred, index) => (
              <div
                key={`pred-${index}`}
                className="absolute w-3 h-3 bg-red-500 rounded-full cursor-pointer hover:scale-150 transition-transform border-2 border-white"
                style={{
                  left: `${((data.length + index) / (data.length + predictionPeriods - 1)) * 100}%`,
                  top: `${100 - scaleValue(pred.value)}%`,
                  transform: 'translate(-50%, -50%)'
                }}
                title={`Prediction ${index + 1}: ${pred.value.toFixed(2)} ${unit} (${(pred.confidence * 100).toFixed(1)}% confidence)`}
              />
            ))}
            ...
            {selectedPoint && (
              <div
                ...
                style={{
                  left: `${(data.findIndex(p => p.timestamp === selectedPoint.timestamp) / (data.length - 1)) * 100}%`,
                  top: `${100 - scaleValue(selectedPoint.value)}%`,
                  transform: 'translate(-50%, -50%)'
                }}
              />
            )}
            ...
            <div className="space-y-2">
              ...
              {prediction && (
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant={prediction.rSquared > 0.7 ? 'default' : 'secondary'}>
                    {prediction.rSquared.toFixed(3)}
                  </Badge>
                </div>
              )}
            </div>
            <div className="space-y-2">
              ...
              {prediction && (
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline">{predictionPeriods}</Badge>
                </div>
              )}
            </div>
          </div>
          ...
          {prediction && showPrediction && (
            <div className="mt-4 p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium">Prediction Summary</span>
              </div>
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="text-gray-600">Next Period:</span>
                  <span className="ml-1 font-medium">{prediction.predictions[0].value.toFixed(2)} {unit}</span>
                </div>
                <div>
                  <span className="text-gray-600">Confidence:</span>
                  <span className="ml-1 font-medium">{(prediction.predictions[0].confidence * 100).toFixed(1)}%</span>
                </div>
                <div>
                  <span className="text-gray-600">Final Prediction:</span>
                  <span className="ml-1 font-medium">{prediction.predictions[prediction.predictions.length - 1].value.toFixed(2)} {unit}</span>
                </div>
                <div>
                  <span className="text-gray-600">Slope:</span>
                  <span className="ml-1 font-medium">{prediction.slope.toFixed(3)}</span>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="mt-4 p-3 bg-gray-50 rounded-lg">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  ...
                  <span className="ml-1 font-medium">{formatDate(selectedPoint.timestamp)}</span>
                </div>
                <div>
                  <span className="text-gray-600">Value:</span>
                  <span className="ml-1 font-medium">{selectedPoint.value.toFixed(2)} {unit}</span>
                </div>
                {selectedPoint.predicted !== undefined && (
                  <div>
                    <span className="text-gray-600">Predicted:</span>
                    <span className="ml-1 font-medium">{selectedPoint.predicted.toFixed(2)} {unit}</span>
                  </div>
                )}
                {selectedPoint.confidence !== undefined && (
                  <div>
                    <span className="text-gray-600">Confidence:</span>
                    <span className="ml-1 font-medium">{(selectedPoint.confidence * 100).toFixed(1)}%</span>
                  </div>
                )}
              </div>
            </div>
