...
export function BubbleChart({ 
  ...
  const getBubbleColor = (bubble: BubbleData) => {
    ...
    if (normalized > 0.7) return '#ef4444' // Red for large
    if (normalized > 0.4) return '#f59e0b' // Yellow for medium
          ...
          >
            ...
            {selectedBubble && (
              <div
                className="absolute"
                style={{
                  left: `${scaleX(selectedBubble.x)}%`,
                  top: `${scaleY(selectedBubble.y)}%`,
                  transform: 'translate(-50%, -50%)'
                }}
              >
                <div
                  className="rounded-full border-4 border-red-500 shadow-lg animate-pulse"
                  style={{
                    width: `${scaleSize(selectedBubble.size) + 8}px`,
                    height: `${scaleSize(selectedBubble.size) + 8}px`
                  }}
                />
              </div>
            )}
          ...
          <div className="mt-4 grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <Badge variant="outline">{minSize.toFixed(2)} - {maxSize.toFixed(2)}</Badge>
              </div>
            </div>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-gray-600">X Range:</span>
                <Badge variant="outline">{minX.toFixed(2)} - {maxX.toFixed(2)}</Badge>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-gray-600">Y Range:</span>
                <Badge variant="outline">{minY.toFixed(2)} - {maxY.toFixed(2)}</Badge>
              </div>
            </div>
          </div>
          ...
          {selectedBubble && (
            <div className="mt-4 p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium">Selected Bubble</span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  <span className="text-gray-600">{xLabel}:</span>
                  <span className="ml-1 font-medium">{selectedBubble.x.toFixed(2)}</span>
                </div>
                <div>
                  <span className="text-gray-600">{yLabel}:</span>
                  <span className="ml-1 font-medium">{selectedBubble.y.toFixed(2)}</span>
                </div>
                <div>
                  <span className="text-gray-600">{sizeLabel}:</span>
                  <span className="ml-1 font-medium">{selectedBubble.size.toFixed(2)}</span>
                </div>
                {selectedBubble.label && (
                  <div>
                    <span className="text-gray-600">Label:</span>
                    <span className="ml-1 font-medium">{selectedBubble.label}</span>
                  </div>
                )}
                {selectedBubble.category && (
                  <div className="col-span-2">
                    <span className="text-gray-600">Category:</span>
                    <span className="ml-1 font-medium">{selectedBubble.category}</span>
                  </div>
                )}
              </div>
            </div>
